...
export const getAllTransactionController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (
      req.query.recurringStatus === "RECURRING" ||
      req.query.recurringStatus === "NON_RECURRING"
    ) {
...
export const getTransactionByIdController = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    return res.status(HTTPSTATUS.OK).json({ message: "Transaction fetched successfully",
      transaction
     });
  } 
);
